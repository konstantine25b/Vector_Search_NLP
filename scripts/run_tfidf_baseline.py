...
def _print_verbose_demo(engine: TfidfSearchEngine, query: str, gold_docs: set[str]) -> None:
    ...
    q_dense = np.asarray(q_row.toarray()).ravel()
    nz = int(np.sum(q_dense != 0))
    print(f"  query_vector: length={q_dense.shape[0]}, nonzeros={nz} (after TF-IDF + L2 norm in sklearn)")
...
def main() -> None:
    ...
    if verbose:
        print("=== TF-IDF baseline walkthrough ===")
    ...
    dm = engine.doc_matrix
    nnz = getattr(dm, "nnz", None)
    voc = len(engine.vectorizer.vocabulary_)
    if verbose:
        print("=== vectorizer (sklearn TfidfVectorizer) ===")
        print(
            f"  lowercase=True  sublinear_tf=True  dtype=float32  "
            f"ngram_range=({args.ngram_min}, {args.ngram_max})  max_features={args.max_features}"
        )
        print(
            "  sklearn also defaults: use_idf=True, smooth_idf=True, norm='l2' "
            "(rows are L2-normalized so dot product equals cosine similarity)."
        )
        print(f"  learned vocabulary size (columns): {voc}")
        print(f"  document_term_matrix shape: {dm.shape[0]} docs x {dm.shape[1]} terms")
        if nnz is not None:
        ...
        print("=== scoring model ===")
        print(
            "  for each query: transform query with the same vectorizer (same IDFs), "
            "then similarity_row = query_row @ document_matrix.T "
            "-> one score per indexed passage, then sort descending."
        )
    if verbose:
        print("tfidf_fit_done")
